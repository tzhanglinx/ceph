...
protected:
  ...
  Inode *cwd;
  int path_walk(const filepath& fp, C_InodeRef *end, bool followsym=true);
...
private:
  ...
  int _do_lookup(Inode *dir, const string& name, C_InodeRef *target);
  int _lookup(Inode *dir, const string& dname, C_InodeRef *target);

  int _link(Inode *in, Inode *dir, const char *name, int uid=-1, int gid=-1, C_InodeRef *inp = 0);
  ...
  int _mkdir(Inode *dir, const char *name, mode_t mode, int uid=-1, int gid=-1, C_InodeRef *inp = 0);
  int _rmdir(Inode *dir, const char *name, int uid=-1, int gid=-1);
  int _symlink(Inode *dir, const char *name, const char *target, int uid=-1, int gid=-1, C_InodeRef *inp = 0);
  int _mknod(Inode *dir, const char *name, mode_t mode, dev_t rdev, int uid=-1, int gid=-1, C_InodeRef *inp = 0);
  int _setattr(Inode *in, struct stat *attr, int mask, int uid=-1, int gid=-1, C_InodeRef *inp = 0);
  ...
  int _create(Inode *in, const char *name, int flags, mode_t mode, C_InodeRef *inp, Fh **fhp,
