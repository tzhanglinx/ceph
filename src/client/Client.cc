...
class C_InodeRef {
  ...
  Inode *in;
public:
  C_InodeRef(Client *c) : client(c), in(NULL) {}
  C_InodeRef(Client *c, Inode *i) : client(c), in(i) {
    if (in)
      in->get();
  }
  C_InodeRef(const C_InodeRef& o) : client(o.client), in(o.in) {
    if (in)
      in->get();
  }
  ~C_InodeRef() {
  ...
  void swap(C_InodeRef& o) {
    assert(client == o.client);
    Inode *tmp = in;
    in = o.in;
    o.in = tmp;
  }
  C_InodeRef& operator=(Inode *in) {
    assert(client);
    C_InodeRef tmp(client, in);
    swap(tmp);
    return *this;
  }
  C_InodeRef& operator=(const C_InodeRef& o) {
    C_InodeRef tmp(o);
    swap(tmp);
    return *this;
  }
  Inode *get() const { return in; }
  Inode &operator*() const { return *in; }
  Inode *operator->() const { return in; }
...
int Client::verify_reply_trace(int r,
			       ...
			       C_InodeRef *ptarget, bool *pcreated,
			       int uid, int gid)
{
...
int Client::_do_lookup(Inode *dir, const string& name, C_InodeRef *target)
{
...
int Client::_lookup(Inode *dir, const string& dname, C_InodeRef *target)
{
  int r = 0;
  Dentry *dn = NULL;
...
int Client::path_walk(const filepath& origpath, C_InodeRef *end, bool followsym)
{
...
int Client::rename(const char *relfrom, const char *relto)
{
  ...
  C_InodeRef fromdir(this), todir(this);
  int r;
			  ...
			  frag_info_t *dirstat, int mask)
{
  ...
  if (r < 0)
    return r;
  r = _getattr(in.get(), mask);
  if (r < 0) {
    ldout(cct, 3) << "stat exit on error!" << dendl;
    return r;
  }
  fill_stat(in.get(), stbuf, dirstat);
...
int Client::chmod(const char *relpath, mode_t mode)
{
  ...
  struct stat attr;
  attr.st_mode = mode;
  return _setattr(in.get(), &attr, CEPH_SETATTR_MODE);
}
...
int Client::utime(const char *relpath, struct utimbuf *buf)
{
  ...
  return _setattr(in.get(), &attr, CEPH_SETATTR_MTIME|CEPH_SETATTR_ATIME);
}
...
int Client::_mknod(Inode *dir, const char *name, mode_t mode, dev_t rdev,
		   int uid, int gid, C_InodeRef *inp)
{
...
int Client::_create(Inode *dir, const char *name, int flags, mode_t mode,
		    C_InodeRef *inp, Fh **fhp, int stripe_unit, int stripe_count,
...
int Client::_rename(Inode *fromdir, const char *fromname, Inode *todir, const char *toname, int uid, int gid)
{
  ...
  if (op == CEPH_MDS_OP_RENAME) {
    ...
    C_InodeRef oldin(this), otherin(this);
...
int Client::_link(Inode *in, Inode *dir, const char *newname, int uid, int gid, C_InodeRef *inp)
{
